...
def extend_bounds2(bounds, extend_min, extend_max, points_per_meter):
    """extend the bounds in all direction by [extend] with a minimum shape of (600,600)"""
    extend = np.random.uniform(extend_min, extend_max)
...
def main():
    data_path = "/home/boosnoel/Documents/eagle_project/datasets/Sammlung/"
    target_path = "/home/boosnoel/Documents/eagle_project/datasets/eagle_1m_ext1/"
    ...
    datapoints_per_meter_read = 4
    datapoints_per_meter_write = 4
    

    extend_min = 200 #in % of the image size that gets padded in each direction
    extend_max = 500
    ...
    for folder in os.listdir(data_path):
        ...
        print("ortho profile: ", ortho_map.profile)
        
        if ortho_map is None or src_polys is None:
            print("could not find all 2 needed files (orho, shp). skip this one: ", folder)
            ortho_map = None
            src_polys = None
            gc.collect()
            continue
        ...
        for iPoly in range(len(src_polys)):
            ...
            bounds_extended = extend_bounds2(shp_bounds, extend_min, extend_max, datapoints_per_meter_read)
            ...
            if np.any(np.array(ortho_data_read_window.flatten())<0):
                print("read window: ", ortho_data_read_window)
                print("ortho profile: ", ortho_map.profile)
                print("bounds calculated: ", bounds_extended)
                continue
                assert(False and "WindowError: shapefile and .tif map do not (fully) overlap")
            ...
            ortho_data = np.vstack((ortho_data, ortho_data, ortho_data))
            ...
            print("data statssss: ", np.amin(ortho_data), np.amax(ortho_data))
            ...
            rgb_channel_normalization(ortho_data)
            print("orhto min max stats 2: ", np.amin(ortho_data), np.amax(ortho_data))

            ortho_data[ortho_data * 0 != 0] = 0

            print("data statsssss: ", np.amin(ortho_data), np.amax(ortho_data))
            ...
            print("write shapes: ", np.shape(ortho_data))
            ...
            #print orhto as dsm
            map_sample_dsm = rasterio.open(path_storage_dsm + str(sample_counter) + ".tif", 'w+', **profile_ortho)
            map_sample_dsm.write(ortho_data.astype(np.uint8))
            ...
            #store mask
            profile_mask = ortho_map.profile.copy()
            profile_mask.update(transform=transform_adjusted)
            profile_mask.update(dtype="uint8", nodata=0, width=width, height=height, count=1)
